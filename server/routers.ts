    ...
    changePassword: publicProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        if (!ctx.user || ctx.user.id !== 0) {
          const admin = await getAdminByUsername(process.env.ADMIN_USERNAME || 'admin');
...
export type AppRouter = typeof appRouter;import { getSessionCookieOptions } from "./_core/cookies.js";
